...
if wp core is-installed --path=/var/www/html; then
    ...
    curl -O https://downloads.wordpress.org/theme/twentynineteen.2.9.zip && \
    ...
    echo "Entering info about user ${WP_ADMIN}"
    wp user meta add ${WP_ADMIN} first_name "Bakyt" #update is applicable?
    wp user meta add ${WP_ADMIN} last_name "Naurzalinov" #same
    wp user meta add ${WP_ADMIN} description "Je suis étudiant à l'École 42."
    ...
    wp post create \
        --post_author=1 \
        --post_title="My first Docker Compose experience." \
